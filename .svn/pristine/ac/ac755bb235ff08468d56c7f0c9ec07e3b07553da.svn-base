 ...
 * 更新描述   ${TODO}
 */
public class SearchListLayoutManagerTV extends LinearLayoutManagerTV {


    private Context mContext;

    public SearchListLayoutManagerTV(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        init(context);
    }

    public SearchListLayoutManagerTV(Context context) {
        super(context);
        init(context);
    }
    ...
    public View onFocusSearchFailed(View focused, int focusDirection, RecyclerView.Recycler recycler,
        ...
        LogUtils.i("onFocusSearchFailed ");
        if (focused != null) {
            int focusPos = 0;
            try {
            ...
            LogUtils.i("focusPos = " + focusPos);

            int nextPos = focusPos + calcOffsetToNextView(focusDirection);
            LogUtils.i("nextPos = " + nextPos);
            View view = findViewByPosition(nextPos);
            if (view == null) {
                LogUtils.i("view is null ");
                View view1 = null;
                int scrollHeight = mContext.getResources().getDimensionPixelSize(R.dimen.search_list_center_item_height);
