package com.ppfuns.ui.view.keyborad;
...
public class SkbContainer extends RelativeLayout implements SoftKeyBoardable {
    ...
    private boolean setKeyCodeEnter(SoftKey softKey) {
        if (softKey == null) {
            LogUtils.e("setKeyCodeEnter softKey is null");
            return false;
        }
    ...
    public boolean onSoftKeyDown(int keyCode, KeyEvent event) {
        ...
        LogUtils.e("onSoftKeyDown keyCode:" + keyCode);
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_DPAD_CENTER:
                ...
                LogUtils.e("onSoftKeyDown softKey:" + softKey);
...
//                int rowIndex = mSoftKeyboardView.getSoftKeyboard().getSelectRow();
//                int rowsSize = mSoftKeyboardView.getSoftKeyboard().getKeyRows().get(rowIndex).getSoftKeys().size();
//                int selectIndex = mSoftKeyboardView.getSoftKeyboard().getSelectIndex();
//
//                LogUtils.i("TAG", "rowIndex=" + rowIndex + "\trowsSize=" + rowsSize + "\tselectIndex=" + selectIndex);
//                if (selectIndex >= rowsSize - 1) {
//                    return false;
//                }

//                SoftKey selectKey = mSoftKeyboardView.getSoftKeyboard().getSelectSoftKey();
//                int currentIndex = selectKey.getNextRightKey().index;
//                int currentRow = selectKey.getNextRightKey().row;
//                LogUtils.i("TAG", "currentIndex=" + currentIndex + "\tcurrentRow=" + currentRow);
                mSoftKeyboardView.setSoftKeyPress(false);
                return actionForKeyEvent(keyCode); // 按键移动.
            case KeyEvent.KEYCODE_DPAD_UP: // 上
                if (mSoftKeyboardView.getSoftKeyboard().getSelectRow() == 0) return false;
                mSoftKeyboardView.setSoftKeyPress(false);
                return actionForKeyEvent(keyCode); // 按键移动.
            case KeyEvent.KEYCODE_DPAD_DOWN: // 下
                ...
                return actionForKeyEvent(keyCode); // 按键移动.
            default:
                LogUtils.e("onSoftKeyDown false keyCode:" + keyCode);
                return false;
        }
        LogUtils.e("onSoftKeyDown true keyCode:" + keyCode);
        return true;
    }
    ...
    public boolean onSoftKeyUp(int keyCode, KeyEvent event) {
        ...
        LogUtils.e("onSoftKeyUp keyCode:" + keyCode);
        if (mSoftKeyboardView != null)
            mSoftKeyboardView.setSoftKeyPress(false);
        switch (keyCode) {
            case KeyEvent.KEYCODE_ENTER:
            ...
            case KeyEvent.KEYCODE_DPAD_DOWN: // 下
            case KeyEvent.KEYCODE_BACK:
                LogUtils.e("onSoftKeyUp true keyCode:" + keyCode);
                return true;
        }
        LogUtils.e("onSoftKeyUp false keyCode:" + keyCode);
        return false;
    }
