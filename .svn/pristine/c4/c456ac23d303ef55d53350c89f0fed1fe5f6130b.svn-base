...
public class XmlKeyboardLoader {
	...
	public SoftKeyboard loadKeyboard(int resourceId) {
		LogUtils.e("loadKeyboard loading ... ...");
		...
		try {
				...
				if (mXmlEventType == XmlResourceParser.START_TAG) {
					...
					if (XMLTAG_KEYBOARD.compareTo(attr) == 0) { // keyboard
						...
						if (softKeyboard == null) {
							LogUtils.e("XMLTAG_ROW softKeyboard null");
							return null;
						}
						...
						if (null == splitter || null == labels) {
							LogUtils.e("XMLTAG_KEYS splitter or labels null");
							return null;
						}
						...
						if (null == softKeyboard) {
							LogUtils.e("XMLTAG_KEY softKeyboard null");
							return null;
						}
			...
			LogUtils.e("loadKeyboard load over");
			return softKeyboard;
		} catch (XmlPullParserException e) {
			e.printStackTrace();
			LogUtils.e("loadKeyboard XmlPullParserException " + e.getMessage());
		} catch (IOException e) {
			e.printStackTrace();
			LogUtils.e("loadKeyboard IOException " + e.getMessage());
		} finally {
			if (xrp != null)
				xrp.close();
		}
